...
#
dataset <- mutate(dataset, physicalb_3group= ifelse(physicalbully=="無", "0", ifelse(rec_bully_physical=="no", "1or2", "repeated")))
        

dataset <- mutate(dataset, relationalb_3group= ifelse(relationalbully=="無", "0", ifelse(rec_bully_relational=="no", "1or2", "repeated")))


dataset <- mutate(dataset, verbalb_3group= ifelse(verbalbully=="無", "0", ifelse(rec_bully_verbal=="no", "1or2", "repeated")))
...
dataset <- mutate(dataset,q5.a=  ifelse( is.na(dataset$q5_a)|dataset$q5_a=="沒有", "沒有","有"))
dataset$q5.a <- factor(dataset$q5.a, levels=c("有","沒有"))
...
dataset_selected<-mutate(dataset_selected, physicalbully_1or0= ifelse(physicalbully=="有",1,0 ))
